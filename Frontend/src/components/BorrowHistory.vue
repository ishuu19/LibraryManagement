        ...
        <div v-else>
            <div class="table-responsive">
                <table class="table table-sm table-hover mb-0">
                    <thead>
                        <tr>
                            <th scope="col" class="sortable-header" @click="sortBy('userEmail')">
                                Email
                                <span v-if="sortField === 'userEmail'" class="sort-icon">
                                    {{ sortOrder === 'asc' ? '↑' : '↓' }}
                                </span>
                            </th>
                            ...
                            <th scope="col" class="sortable-header" @click="sortBy('dueDate')">
                                Due Date
                            ...
                            <th scope="col" class="sortable-header" @click="sortBy('status')">
                                Status
                    ...
                    <tbody>
                        <tr v-for="borrow in sortedBorrows" :key="borrow._id">
                            ...
                            <td>{{ borrow.status === 'active' ? 'Active' : 'Returned' }}</td>
                        </tr>
            ...
            <div v-if="totalItems > 0" class="d-flex justify-content-between align-items-center mt-2 pt-2 border-top">
                <div class="text-muted small">
...
const fetchBorrowHistory = async () => {
    ...
    try {
        ...
        const response = await fetch(
            `${API_BASE}/${props.bookId}/borrow-history?page=${currentPage.value}&limit=${perPage.value}`,
            {
            headers: {
...
const filteredBorrows = computed(() => {
    ...
    return borrows.value.filter(borrow => {
        const email = borrow.user?.email?.toLowerCase() || '';
